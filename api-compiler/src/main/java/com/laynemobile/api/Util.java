...
import static com.laynemobile.api.Constants.BaseApi;
import static com.laynemobile.api.Constants.Builder;
import static com.laynemobile.api.Constants.List;
import static com.laynemobile.api.Constants.PACKAGE;
import static com.laynemobile.api.Constants.RequestProcessor;
import static com.laynemobile.api.Constants.RequestProcessorBuilder;
import static com.laynemobile.api.Constants.Source;
import static com.laynemobile.api.Constants.SourceBuilder;
...
final class Util {
    ...
    static TypeName coallesceParamType(ClassName rawType, List<? extends TypeName> paramTypes,
        ...
        for (TypeName paramType : paramTypes) {
            ApiLog.d(TAG, "paramTypeName: %s", paramType);
            if (!(paramType instanceof TypeVariableName)) {
                if (paramType instanceof ParameterizedTypeName) {
                    ParameterizedTypeName ptn = (ParameterizedTypeName) paramType;
                    paramType = coallesceParamType(ptn.rawType, ptn.typeArguments, containerType, typeUtils);
                } else {
                    ApiLog.d(TAG, "Don't know how to handle typeName: %s", paramType);
                }
                types.add(paramType);
                continue;
            }
            ApiLog.d(TAG, "paramTypeBounds: %s", ((TypeVariableName) paramType).bounds);
            ...
            if (bounds.size() != 0) {
                ...
                while (containerTypesIterator.hasNext()) {
                    ...
                    if (found != null) {
                        ApiLog.d(TAG, "found super type: %s", found);
        ...
        ApiLog.d(TAG, "finalTypes: %s", types);
    ...
    private static TypeName findTypeName(TypeMirror type, List<? extends TypeName> bounds, Types typeUtils) {
        ...
        ApiLog.d(TAG, "checking type: %s", typeName);
        if (contains(bounds, typeName)) {
            ApiLog.d(TAG, "found type: %s", typeName);
            return typeName;
        }
