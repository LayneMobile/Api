...
import api.annotations.GenerateApiBuilder;
import api.internal.ApiLog;
import api.sources.SourceHandlerBuilder;

import static api.Constants.NonNull;
import static api.Constants.Override;
import static api.Constants.SourceModule;
...
final class ApiBuilder {
    ...
    ApiBuilder(TypeElement api, Types typeUtils) {
        ...
        ApiLog.d(TAG, "superTypeParams: %s", superTypeParams);
...
//        javaFile.writeTo(System.out);
    ...
    private JavaFile build() {
        ...
        ApiLog.d(TAG, "GenerateApiBuilder element: %s", api.getQualifiedName());
        GenerateApiBuilder annotation = api.getAnnotation(GenerateApiBuilder.class);
        try {
            Class<? extends SourceHandlerBuilder>[] builders = annotation.value();
